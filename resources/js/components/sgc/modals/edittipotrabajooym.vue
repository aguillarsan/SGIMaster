<template>
    <div>
        <div aria-hidden="true" class="modal inmodal" id="EditTipoTrabajo" role="dialog" tabindex="-1" data-backdrop="static">
...
export default {
    ...
    mounted: function() { /* Agrega mounted y con el siguiente contenido */
        let self = this;
        EventBus.$on('sgc.modalEditTipoTrabajo', function (solicitud) {
    ...
    methods:{
        ...
        ShowModal:function(solicitud){
            ...
            this.getTipoTrabajo()
            $('#EditTipoTrabajo').modal('show');
        },
        getTipoTrabajo:function(){
            axios.get('/GetTipoTrabajo').then((response) => {
        ...
        updateProveedorRequest:function(){
           ...
           if(this.tipo_trabajo_id != ''){
                ...
                axios.get(`/tipoTrabajoupdateSgc?solicitud_id=${this.solicitud_id}&tipo_trabajo_id=${this.tipo_trabajo_id}`).then((response)=>{
                    $('#EditTipoTrabajo').modal('hide');
                    this.loadFunction()
                    this.cleanthisData()
